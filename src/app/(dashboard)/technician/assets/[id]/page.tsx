...
export default async function TechnicianAssetDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto text-left">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          <QRScannerModal />
          <Link href={`/technician/fault-report?assetId=${asset.id}`}>
            <Button className="shadow-sm bg-destructive hover:bg-destructive/90 text-white font-semibold">Report Issue</Button>
          </Link>
        </div>
      </div>
        ...
        <Card className="shadow-sm border-slate-200 col-span-2">
          ...
          <CardContent className="pt-6 grid sm:grid-cols-2 gap-x-8 gap-y-6">
            <div>
              ...
              <div className="font-semibold text-slate-900">{asset.serialNumber}</div>
            </div>
            <div>
              <div className="text-sm text-slate-500 mb-1">Department</div>
              <div className="font-semibold text-slate-900">{asset.department}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="font-semibold text-slate-955 flex items-center gap-2">
            ...
            <div>
              ...
              <div className="font-semibold text-slate-900">{asset.supplier}</div>
            </div>
            <div>
              <div className="text-sm text-slate-500 mb-1">Frequency check</div>
              <div className="font-semibold text-slate-900">{asset.maintenanceFrequency}</div>
            </div>
          </CardContent>
        </Card>
